...
def produce_weight(data,K):
    ...
    return weight
# 更新隶属度和聚类中心
def cal_weight(K,data,center,weight, m = 2, a = 3):
    dx = [-1, 0, 1, -1, 1, -1, 0, 1]
    dy = [-1, -1, -1, 0, 0, 1, 1, 1]
    row = data.shape[0]
    column = data.shape[1]
    distance = np.zeros((row, column, K))
    neighbor_dist = np.zeros((row, column, K))
    miu_fenzi = np.zeros((row, column, K, 3))
    for i in range(row):
        for j in range(column):
            for k in range(K):
                distance[i][j][k] = pow(np.linalg.norm(data[i][j] - center[k]), 2)
                for r in range(8):
                    index_x = i + dx[r]
                    index_y = j + dy[r]
                    if i + dx[r] < 0 or i + dx[r] >= row:
                        index_x = i
                    if j + dy[r] < 0 or j + dy[r] >= column:
                        index_y = j
                    neighbor_dist[i][j][k] += (a/8)* pow(np.linalg.norm(data[index_x][index_y] - center[k]), 2)
                miu_fenzi[i][j][k] = pow(distance[i][j][k] + neighbor_dist[i][j][k], 1/(m-1))
    miu_fenmu = np.sum(miu_fenzi, axis=2) + 0.01
    tmp = np.zeros((row, column, K))
    # print(miu_fenzi[0][0])

    for i in range(row):
        for j in range(column):
            for k in range(K):
                tmp[i][j][k] = miu_fenzi[i][j][k][0]/miu_fenmu[i][j][0] #由于
                # t = 0
                # for l in range(3):
                #     t += tmp[i][j][k][l]
                # tmp[i][j][k][0] = t/3
                weight[i][j][k] = tmp[i][j][k]
    return weight
def cal_center(data,weight,center, m = 2, a = 3):
    dx = [-1, 0, 1, -1, 1, -1, 0, 1]
    dy = [-1, -1, -1, 0, 0, 1, 1, 1]
    row = data.shape[0]
    column = data.shape[1]
    K = weight.shape[2]
    b = np.sum(weight, axis=0)
    c = np.sum(b, axis=0)
    weight_sum = (1 + a) * c

    sum_neighbor = np.zeros((row, column, 3))
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            for r in range(8):
                ...
                if j + dy[r] < 0 or j + dy[r] >= column:
                    index_y = j
                sum_neighbor[i][j] += (a/8) * data[index_x][index_y]
    pixel_information = np.zeros((row, column, K, 3))
    for k in range(K):
        for i in range(row):
            for j in range(column):
                pixel_information[i][j][k] += pow(weight[i][j][k], m) *(data[i][j] + sum_neighbor[i][j])
    c2 = np.zeros((K,3))
    for i in range(row):
        for j in range(column):
            for k in range(K):
                c2[k] += pixel_information[i][j][k]
    for k in range(K):
        center[k] = c2[k][0]/weight_sum[k]
    return center
